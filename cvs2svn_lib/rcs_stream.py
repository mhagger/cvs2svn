...
class RCSStream:
  ...
  def generate_blocks(self, edits):
    ...
    generate_edits().  Generate a tuple (COMMAND, START, COUNT,
    [LINE,...]) for each block implied by EDITS when applied to the
    current contents of SELF.  Blocks consist of ed commands and copy
    blocks:
  ...
  def apply_diff(self, diff):
    ...
    for (command, start, count, lines) in blocks:
      if command == 'c':
