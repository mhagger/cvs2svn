...
Process each tag in a git repository. If the tagged commit is tree-wise
identical with another tagged commit, the tag is moved to point at the
other commit (i.e. tags pointing at identical content will all point at a
single tag fixup commit).

Furthermore, if one of the parents of the tag fixup commit is identical to the
tag fixup commit itself, then the tag is moved to the parent.
...
# Cache trees we have already seen, and that are suitable targets for moved tags
...
def try_to_move_tag(tag, commit, tree, parents):
    ...
    for p in parents:
        if p not in parent_cache: # Not in cache
            parent_cache[p] = resolve_commit(p)
        p_tree = parent_cache[p]
        if tree == p_tree: # We can move tag to parent p
...
while True: # While get_tag_info process is still running
    for line in get_tag_info.stdout:
        ...
        if objtype == "tag":
            ...
            tree    = tree_alt
            parents = parents_alt
        elif objtype != "commit": continue

        if subject.startswith("This commit was manufactured by cvs2svn") or not subject:
            # We shall try to move this tag, if possible
            parent_list = []
            if parents: parent_list = parents.split(" ")
            for p in parent_list: assert len(p) == 40
    ...
    if get_tag_info.poll() is not None: break # Break if no longer running
